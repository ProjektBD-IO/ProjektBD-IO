...
public interface GifRepository extends JpaRepository<Gif,Long> {
    @Query("select g, c.category_name from Gif g JOIN FETCH Categories c on g.id_category=c.id_category where g.tags like %:tag% and gif_type=true order by add_date desc")
